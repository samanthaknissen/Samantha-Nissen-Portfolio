...
function the_posts_navigation() {
	...
	<nav class="navigation posts-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php _e( 'Posts navigation', 'portfolio' ); ?></h2>
		<div class="nav-links">
...
function the_post_navigation() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php _e( 'Post navigation', 'portfolio' ); ?></h2>
		<div class="nav-links">
			<?php
...
function the_archive_title( $before = '', $after = '' ) {
	if ( is_category() ) {
		$title = sprintf( __( 'Category: %s', 'portfolio' ), single_cat_title( '', false ) );
	} elseif ( is_tag() ) {
		$title = sprintf( __( 'Tag: %s', 'portfolio' ), single_tag_title( '', false ) );
	} elseif ( is_author() ) {
		$title = sprintf( __( 'Author: %s', 'portfolio' ), '<span class="vcard">' . get_the_author() . '</span>' );
	} elseif ( is_year() ) {
		$title = sprintf( __( 'Year: %s', 'portfolio' ), get_the_date( _x( 'Y', 'yearly archives date format', 'portfolio' ) ) );
	} elseif ( is_month() ) {
		$title = sprintf( __( 'Month: %s', 'portfolio' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'portfolio' ) ) );
	} elseif ( is_day() ) {
		$title = sprintf( __( 'Day: %s', 'portfolio' ), get_the_date( _x( 'F j, Y', 'daily archives date format', 'portfolio' ) ) );
		...
		$title = sprintf( __( 'Archives: %s', 'portfolio' ), post_type_archive_title( '', false ) );
	} elseif ( is_tax() ) {
		$tax = get_taxonomy( get_queried_object()->taxonomy );
		/* translators: 1: Taxonomy singular name, 2: Current taxonomy term */
		$title = sprintf( __( '%1$s: %2$s', 'portfolio' ), $tax->labels->singular_name, single_term_title( '', false ) );
	} else {
